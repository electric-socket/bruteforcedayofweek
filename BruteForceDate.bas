...
' This is used if you might have to get the day of the week
' from 1800 to 9999. There is a simpler one for less wide uses
'
Option _Explicit
Const Saturday = 0
...
Dim Shared YD(StartYear To LimitYear, December, 31) As Integer
...
Print DayNames(YD(Year, Month, Day)); ", "; MonthNames(Month); Str$(Day); ","; Year; "at";

' If military format is OK, just print t$
'' Use either:
' Print " ";T$
'' or
Print Str$(Hour); ":"; Right$("0" + LTrim$(Str$(Minute)), 2); ":"; Right$("0" + LTrim$(Str$(Second)), 2)
